...
#### Video Demo:  [My youtube link](https://youtu.be/zxJD9MgvVlg)
#### Description: 
- The seller has:
...
We solve it through searching for solution nodes over a tree generated by a mini rule-based system.
...
- The capacity of the first container is 11 liters. (Y1 = 11)
- The capacity of the second container is 8 liters. (Y2 = 8)
- The customer wants to buy 6 liters of oil. (Z = 6)

#### Example 2
X = 16, Y1 = 10, Y2 = 6, Z = 5
- The seller has 16 liters of oil. (X = 16)
- The capacity of the first container is 10 liters. (Y1 = 10)
- The capacity of the second container is  6 liters. (Y2 = 6)
...
- We use the technique of rule-based system in Knowledge Engineering to generate the tree. 
...
class Node:
    def __init__(self, 
            id, parent_id, n_storage, container_1, container_2, path, rule, level):
        ...
        self.path = path                # list of nodes from root to this: list
...
def create_tree(storage, capacity_1, capacity_2, needed):
    ...
    Create the first value of lst_containers list.
    Create the dictionary tree and search for the solution nodes through this tree.
    The list lst_containers is used for quick checking if a pseudo node can become a new node.
    Return the result list with and the tree dictionary.
    """    
    tree = {}
    lst_containers = []
